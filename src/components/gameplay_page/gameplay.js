...
class Gameplay extends Component {
  ...
  playerClick = (idClick) => {
    switch (idClick) {
      case "stone":
        this.setState({ selectStone: true });
        break;

      case "paper":
        this.setState({ selectPaper: true });
        break;

      case "scissors":
        this.setState({ selectScissors: true });
        break;
  ...
  comSelect = async (idClick) => {
    ...
    switch (computerChoice) {
      case "com-stone":
        this.setState({ comSelectStone: true });
        break;

      case "com-scissors":
        this.setState({ comSelectScissors: true });
        break;

      case "com-paper":
        this.setState({ comSelectPaper: true });
        break;
  ...
  postData = (classId) => {
    ...
    UserService.postHistory(this.state.token, {
      user_id: this.state.id,
      game_id: 7,
      high_score: gameResult,
    })
      .then((result) => {
        console.log(result);
      })
  ...
  render() {
    ...
    return (
      <div className="gameplay pt-2">
        <Header quitGame={() => this.setState({ modalsConfirm: true })} />
        <Container>
          <Row>
            <Col>
              <div className="d-flex flex-column text-center">
                ...
                <figure className={`${selectStone ? "chosen" : ""}`}>
                  <input
                    className={`img-assets ${hover}`}
                    alt="stone"
                    type="image"
                    disabled={!imgHover}
                    src={Stone}
                    onClick={() => this.playerClick("stone")}
                  ></input>
                </figure>
                <figure className={`${selectPaper ? "chosen" : ""}`}>
                  <input
                    ...
                    disabled={!imgHover}
                    src={Paper}
                    onClick={() => this.playerClick("paper")}
                  ></input>
                </figure>
                <figure className={`${selectScissors ? "chosen" : ""}`}>
                  <input
                    ...
                    disabled={!imgHover}
                    src={Scissors}
                    onClick={() => this.playerClick("scissors")}
                  ></input>
                </figure>
              </div>
            </Col>
            <Col>
              ...
              <Reset resetState={resetState} reset={() => this.reset()} />
            </Col>
            <Col>
              <div className="d-flex flex-column text-center">
